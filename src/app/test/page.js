...
export default function LoginPage() {
    ...
    const [data, setData] = useState({"ofwief" : "ofwief"});
  ...
  return (
    <div>
      ...
      <button onClick={handleLogin} disabled={!deviceInfo}>
              Login with Device Info
              
          </button>
          <span>{ `${Object.entries(deviceInfo).map(([key, value]) => `${key}: ${value}`).join(', ')}`}</span>
    </div>
  );
}
